...
import maplibregl, { FeatureCollection, Feature, Geometry, GeoJsonProperties, GeoJSONSource } from "maplibre-gl";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const regionFeature: Feature<Geometry, GeoJsonProperties> | undefined = regionsGeo.features?.find(
        ...
        const fitToFeature = (feature: Feature<Geometry, GeoJsonProperties>, pad = 56) => {
          try {
            const coords: number[][] = [];
            const push = (geom: any) => {
        ...
        map.on("load", () => {
          ...
          // Explicitly typed FeatureCollection with 'type' literal for TS compatibility
          const regionFC: FeatureCollection = regionFeature
            ? { type: "FeatureCollection" as const, features: [regionFeature] }
          ...
          const regionCountriesFC: FeatureCollection = {
          ...
          map!.addSource("countries", { type: "geojson", data: regionCountriesFC });
          ...
          if (viewMode === "country" && selectedCountry) {
            const cf = regionCountriesFC.features.find((f: any) => f?.properties?.NAME === selectedCountry);
            if (cf) {
